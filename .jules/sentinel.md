...
**Vulnerability:** The application used `Math.random()` to generate sensitive identifiers, including API keys and reservation codes. `Math.random()` is not cryptographically secure and its output can be predictable.
**Learning:** Standard PRNGs are suitable for UI logic but dangerous for security-sensitive tokens. In a web environment, `globalThis.crypto.getRandomValues()` should always be used for identifier generation.
**Prevention:** Implement standardized, secure utility functions for random hex and numeric generation and enforce their use for all security-sensitive identifiers.
