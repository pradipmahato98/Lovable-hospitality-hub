...
**Learning:** Developers often use `Math.random()` for "random-looking" strings without considering the security implications when those strings are used as secrets or for isolation.
**Prevention:** Always use `globalThis.crypto.getRandomValues()` for security-sensitive random data. Centralize these in a utility to make secure generation easy to adopt.
