...
**Vulnerability:** Use of `Math.random()` for generating API keys, reservation codes, and file paths. `Math.random()` is PRNG and not cryptographically secure, making generated values potentially predictable.
**Learning:** Developers often reach for `Math.random()` for convenience when a "random-looking" string is needed, without considering the security implications of predictability for sensitive identifiers.
**Prevention:** Implement and use a centralized cryptographically secure utility (using `crypto.getRandomValues`) for all security-sensitive random generation.
