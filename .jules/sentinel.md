...
**Vulnerability:** Use of `Math.random()` for generating sensitive identifiers like API keys and realtime channel IDs. `Math.random()` is not cryptographically secure and can be predictable.
**Learning:** Standard JavaScript `Math.random()` is unsuitable for security-sensitive contexts. Rejection sampling is required when using `crypto.getRandomValues()` to generate numbers within a specific range to avoid modulo bias.
**Prevention:** Use `globalThis.crypto.getRandomValues()` for all security-sensitive random generation and provide centralized, tested utilities for common patterns (hex strings, bounded integers).
