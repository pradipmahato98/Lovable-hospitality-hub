...
**Vulnerability:** Use of `Math.random()` for generating API keys and realtime channel IDs. `Math.random()` is not cryptographically secure and can be predictable.
**Learning:** Standard library random functions are often used by default for convenience, but they are unsuitable for security-sensitive tokens where collision resistance and unpredictability are required.
**Prevention:** Always use the Web Crypto API (`globalThis.crypto.getRandomValues`) for generating API keys, session tokens, or any identifier that requires security. Centralize these in a `generateSecureHex` utility.
