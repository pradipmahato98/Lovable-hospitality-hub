...
## 2026-02-11 - Insecure Random Identifier Generation
**Vulnerability:** Use of `Math.random()` for security-sensitive identifiers including API keys, reservation codes, and transaction numbers. `Math.random()` is not cryptographically secure and can be predictable.
**Learning:** Legacy code often defaults to `Math.random()` for convenience. In a mixed browser/Node environment (like Vitest), a centralized utility and a global test mock are necessary to ensure security while maintaining testability.
**Prevention:** Always use `globalThis.crypto.getRandomValues()` for identifiers that require unpredictability. Provide a standard library utility (`generateSecureHex`, `generateSecureNumber`) to discourage fallback to insecure methods.
