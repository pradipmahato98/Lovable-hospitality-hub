...
## 2026-05-22 - Insecure PRNG for Sensitive Identifiers
**Vulnerability:** The application used `Math.random()` to generate sensitive identifiers, including system API keys, POS transaction numbers, and avatar filenames. `Math.random()` is not cryptographically secure and can be predictable.
**Learning:** Common JavaScript/TypeScript idioms for generating "random" strings (like `Math.random().toString(36)`) are frequently used for convenience but are unsuitable for security-sensitive tokens or identifiers that require high entropy and unpredictability.
**Prevention:** Standardize on cryptographically secure random utilities (`crypto.getRandomValues`) for all sensitive identifier generation. Centralizing these in a utility library ensures consistency and eases security audits.
