...
**Vulnerability:** Use of `Math.random()` for security-sensitive operations including API key generation, SKU generation, and image filenames. `Math.random()` is not cryptographically secure and can lead to predictable identifiers or collisions.
**Learning:** Legacy code and "quick fixes" often rely on `Math.random()` for convenience, which creates a cumulative security debt across the application.
**Prevention:** Centralize cryptographically secure PRNG utilities (`generateSecureHex`, `generateSecureNumber`) in a core library and enforce their use for all non-trivial random generation.
