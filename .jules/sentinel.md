...
**Vulnerability:** Use of `Math.random()` for security-sensitive identifiers like API keys, reservation codes, and folio numbers.
**Learning:** `Math.random()` is not cryptographically secure and can be predictable. For identifiers that act as secrets or need to be unguessable, a CSPRNG must be used.
**Prevention:** Implement and use a centralized utility (e.g., `generateSecureHex`, `generateSecureNumber`) that wraps `crypto.getRandomValues()` with rejection sampling to ensure uniform distribution and cryptographic strength.
