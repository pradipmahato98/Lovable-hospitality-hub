...
**Vulnerability:** Use of `Math.random()` for security-sensitive identifiers (API keys, reservation codes, folio numbers). `Math.random()` is not cryptographically secure and can be predictable.
**Learning:** Default random utilities in many languages/environments are PRNGs designed for speed or distribution, not security. For any value that needs to be unique and unpredictable (like a secret or a unique ID), a CSPRNG must be used.
**Prevention:** Standardize on cryptographically secure random utilities (e.g., `globalThis.crypto.getRandomValues`) for all identifier generation across the codebase.
