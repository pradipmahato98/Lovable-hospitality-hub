...
**Vulnerability:** The application used `Math.random()` for generating API keys, transaction numbers, and reservation codes. `Math.random()` is not cryptographically secure and its output can be predicted if the internal state of the PRNG is recovered.
**Learning:** Developers often reach for `Math.random()` for "random-looking" strings without considering the security implications of predictability in a multi-user environment.
**Prevention:** Always use `globalThis.crypto.getRandomValues()` (CSPRNG) for any value that must be unique and unpredictable, such as API keys, session tokens, or transaction IDs. Centralize these into a `generateSecureRandom` utility.
