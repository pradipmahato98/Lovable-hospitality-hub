...
**Vulnerability:** Use of `Math.random()` for generating API keys in the Admin Console.
**Learning:** `Math.random()` is not cryptographically secure and its output is predictable. Using it for sensitive identifiers like API keys is a critical security flaw.
**Prevention:** Use `globalThis.crypto.getRandomValues()` for all security-sensitive random generations.
