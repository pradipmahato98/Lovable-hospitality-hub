...
## 2026-05-02 - Insecure PRNG for Sensitive Identifiers
**Vulnerability:** The application used `Math.random()` to generate API keys, reservation codes, and transaction numbers. `Math.random()` is not cryptographically secure and its output can be predicted, potentially allowing attackers to guess sensitive identifiers.
**Learning:** Standard JavaScript `Math.random()` is designed for performance, not security. For any identifier that needs to be unpredictable (secrets, IDs, tokens), a cryptographically secure pseudo-random number generator (CSPRNG) must be used.
**Prevention:** Implement standardized utility functions that wrap `globalThis.crypto.getRandomValues()` and enforce their use across the codebase for all security-sensitive random generation.
