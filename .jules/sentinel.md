...
**Vulnerability:** The application used `Math.random()` for generating sensitive identifiers like API keys and realtime channel IDs. `Math.random()` is not cryptographically secure and can be predictable.
**Learning:** Developers often use `Math.random()` for convenience when a "random string" is needed, but it should never be used for anything requiring unpredictability or security.
**Prevention:** Always use a cryptographically secure PRNG (like `crypto.getRandomValues`) for sensitive data. Established a shared `generateSecureHex` utility to standardize secure random generation.
