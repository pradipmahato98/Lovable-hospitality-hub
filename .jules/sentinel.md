...
**Vulnerability:** Use of `Math.random()` for generating API keys, reservation codes, and transaction numbers. `Math.random()` is not cryptographically secure and is predictable.
**Learning:** Developers often use `Math.random()` for simple IDs without considering the security implications when those IDs are used as secrets or unique references in multi-user systems.
**Prevention:** Always use `globalThis.crypto.getRandomValues()` (CSPRNG) for generating any identifier that needs to be unique, unguessable, or used for security purposes.
