...
**Vulnerability:** The application used `Math.random()` to generate API keys and POS transaction suffixes, making these sensitive identifiers potentially predictable.
**Learning:** Developers often reach for `Math.random()` due to its simplicity, unaware that it is not cryptographically secure and can lead to token prediction vulnerabilities.
**Prevention:** Standardize on `globalThis.crypto.getRandomValues()` for all sensitive random data. Provide a shared utility like `generateSecureHex` and ensure a test environment mock is available for browser-compatible crypto APIs.
