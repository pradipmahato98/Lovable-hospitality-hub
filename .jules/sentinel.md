...
**Vulnerability:** Use of `Math.random()` for sensitive identifiers like API keys and system SKUs, which is predictable and unsuitable for security-sensitive contexts.
**Learning:** Legacy code or quick implementations often default to `Math.random()` because it's built-in and easy to use, ignoring its lack of cryptographic strength.
**Prevention:** Standardize on cryptographically secure random value generation (e.g., `globalThis.crypto.getRandomValues()`) for any value that must be unique or unguessable.
