...
**Vulnerability:** The application used `Math.random()` to generate API keys, reservation codes, and transaction numbers. `Math.random()` is not cryptographically secure and can be predictable.
**Learning:** Developers often use `Math.random()` for "quick" ID generation without considering the security implications of predictability in a multi-tenant or high-traffic environment.
**Prevention:** Always use `globalThis.crypto.getRandomValues()` or `crypto.randomUUID()` for generating identifiers that require uniqueness and unpredictability. Standardize these in a shared utility library.
