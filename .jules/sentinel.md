...
**Vulnerability:** Use of `Math.random()` for generating API keys, internal SKUs, and file names, making them cryptographically weak and predictable.
**Learning:** Standard PRNGs like `Math.random()` are not suitable for security-sensitive generation. Even for non-secret IDs like SKUs, predictability can lead to scraping or enumeration attacks.
**Prevention:** Implement and enforce the use of cryptographically secure random utilities (wrapping `crypto.getRandomValues`) for any identifier that must be unique and non-predictable.
