...
**Vulnerability:** API keys and other sensitive identifiers were being generated using `Math.random()`, which is cryptographically insecure and predictable.
**Learning:** Standard JavaScript `Math.random()` does not provide enough entropy for security-sensitive tokens. API keys generated this way are vulnerable to brute-force or prediction attacks.
**Prevention:** Always use `globalThis.crypto.getRandomValues()` or `crypto.randomUUID()` for generating secrets, API keys, or unique tokens. Centralize these in a security utility file.
