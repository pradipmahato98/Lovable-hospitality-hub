 ...
 *
 * Using verified lookup data (BS 2000 - 2100).
...
const REF_AD_DATE_UTC = Date.UTC(1943, 3, 14);
...
export function adToBs(date: Date | string): string {
  ...
  while (true) {
    const daysInYear = (bsMonthDays[year] || [365]).reduce((a, b) => a + b, 0);
    if (diff < daysInYear) break;
    diff -= daysInYear;
    year++;
    if (year > 2100) return ""; // Beyond supported range
  }
...
export function bsToAd(bsDate: string): string {
  ...
  if (year < REF_BS_YEAR || !bsMonthDays[year] || month < 0 || month >= 12 || day < 1 || day > bsMonthDays[year][month]) return "";
