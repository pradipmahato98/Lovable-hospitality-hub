...
export function CategoriesTab() {
  ...
  const [form, setForm] = useState({ category_name: "", description: "", parent_category: "", sku_prefix: "" });

  const getItemCount = (catId: string) => items.filter((i) => i.category_id === catId).length;
  const getParentName = (parentId: string | null) => categories.find((c) => c.id === parentId)?.category_name || "-";
  ...
  const handleSave = async () => {
    try {
      ...
      setOpen(false);
      setEditId(null);
      setForm({ category_name: "", description: "", parent_category: "", sku_prefix: "" });
    } catch (error: any) {
    ...
    <Card variant="elevated">
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) { setEditId(null); setForm({ category_name: "", description: "", parent_category: "", sku_prefix: "" }); } }}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2"><Label>Name *</Label><Input value={form.category_name} onChange={(e) => setForm({ ...form, category_name: e.target.value })} /></div>
                ...
                <div className="space-y-2"><Label>Parent Category</Label>
                  <Select value={form.parent_category} onValueChange={(v) => setForm({ ...form, parent_category: v === "none" ? "" : v })}>
                    <SelectTrigger><SelectValue placeholder="None (top level)" /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">None (top level)</SelectItem>
                      {categories.filter((c) => c.id !== editId).map((c) => <SelectItem key={c.id} value={c.id}>{c.category_name}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
              </div>
              <DialogFooter>
                ...
                <Button onClick={handleSave} disabled={!form.category_name || createCategory.isPending || updateCategory.isPending}>
