...
export function generateSecureRandomNumber(min: number, max: number): number {
  ...
  // If range is large enough to cover the entire 32-bit space,
  // we can just use the random value directly without bias.
  if (range >= 4294967296) {
    const array = new Uint32Array(1);
    globalThis.crypto.getRandomValues(array);
    return rangeMin + array[0];
  }
  ...
  return rangeMin + (randomValue % range);
}
